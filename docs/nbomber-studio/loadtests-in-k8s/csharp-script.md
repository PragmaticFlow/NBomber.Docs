...
Currently, Studio supports only a single script file per project. We believe that code should live in Git, not in Studio. That's another reason why we recommend the Docker project type for production load tests.
:::
...
Pay attention to adding TimescaleDbSink: NBomberRunner.WithReportingSinks(new TimescaleDbSink())
...
It's important to keep it disabled because NBomber functionality depends on it.
*/
...
Now that everything is ready, click **Save** and then run your test. Studio creates a Job that deploys all required environment dependencies (such as the NATS message broker) and passes through configuration like the license key (if provided in Studio) and the TimescaleDB connection string. Once the test finishes, the Job cleans up all resources automatically.
